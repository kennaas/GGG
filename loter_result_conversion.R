...
innerFile = file("Data/loter/Run 3/Loter input 3/inner_phased3.vcf.gz",open = "r")
innerLine  = scan(innerFile, nlines = 1, skip = 8,
                  what = character(), quiet = TRUE)
close(innerFile)
innerInds = innerLine[-(1:9)]

outerFile = file("Data/loter/Run 3/Loter input 3/outer_phased3.vcf.gz",open = "r")
outerLine  = scan(outerFile, nlines = 1, skip = 8,
                  what = character(), quiet = TRUE)
close(outerFile)
outerInds = outerLine[-(1:9)]

otherFile = file("Data/loter/Run 3/Loter input 3/other_phased3.vcf.gz",open = "r")
otherLine  = scan(otherFile, nlines = 1, skip = 8,
                  what = character(), quiet = TRUE)
close(otherFile)
otherInds = otherLine[-(1:9)]

admixedFile = file("Data/loter/Run 3/Loter input 3/admixed_phased3.vcf.gz",open = "r")
admixedLine  = scan(admixedFile, nlines = 1, skip = 8,
                  what = character(), quiet = TRUE)
close(admixedFile)
admixedInds = admixedLine[-(1:9)]
SNPs = fread(file = "Data/loter/Run 3/Loter input 3/admixed_phased3.vcf.gz",
              select = 3)$ID
...
AOuter = BGData(geno =
                 initFileBackedMatrix(length(inds), 2 * length(SNPs),
                                       folderOut = "Data/loter/Run 3/AOuter",
                                       outputType = "boolean"),
                pheno = data.frame(ID = inds))
AInner = BGData(geno =
                 initFileBackedMatrix(length(inds), 2 * length(SNPs),
                                       folderOut = "Data/loter/Run 3/AInner",
                                       outputType = "boolean"),
                pheno = data.frame(ID = inds))
AOther = BGData(geno =
                initFileBackedMatrix(length(inds), 2 * length(SNPs),
                                       folderOut = "Data/loter/Run 3/AOther",
                                       outputType = "boolean"),
                pheno = data.frame(ID = inds))
...
admixedFile = file("Data/loter/Run 3/loter_result3.txt",open = "r")
for (row in i[1]:i[2]) {
...
save(AInner, file = "Data/loter/Run 3/AInner/AInner.RData")
save(AOuter, file = "Data/loter/Run 3/AOuter/AOuter.RData")
save(AOther, file = "Data/loter/Run 3/AOther/AOther.RData")
