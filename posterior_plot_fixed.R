...
get_posterior = function(model, effect) {
  return(inla.smarginal(model$marginals.fixed[[effect]]))
}

make_post_plot = function(response, legend) {
  outerFixedGen = get_posterior(get(paste0(response, "RioVR")), 6)
  otherFixedGen = get_posterior(get(paste0(response, "RioVR")), 7)
  outerFixedPed = get_posterior(get(paste0(response, "GGPed")), 6)
  otherFixedPed = get_posterior(get(paste0(response, "GGPed")), 7)
  
  plot = ggplot() +
    geom_line(data = subset(data.frame(outerFixedGen), y > 0.02), 
              aes(x, y, color = "outer", linetype = "Genomic"),
              size = 1) +
    geom_line(data = subset(data.frame(otherFixedGen), y > 0.02),
              aes(x, y, color = "other", linetype = "Genomic"),
              size = 1) +
    geom_line(data = subset(data.frame(outerFixedPed), y > 0.02),
              aes(x, y, color = "outer", linetype = "Pedigree"),
              size = 1) +
    geom_line(data = subset(data.frame(otherFixedPed), y > 0.02),
